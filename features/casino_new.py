...
# Setup casino logger
logger = logging.getLogger('casino_new')
    ...
    async def _handle_payout(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if payout > 0:
                ...
                net_change = payout - self.game.bet_amount
                logger.info(f"💰 Casino payout: User {self.game.user_id} bet {self.game.bet_amount}, payout {payout}, net {net_change:+}")
            else:
                # Thua - không cần làm gì vì tiền đã bị trừ ở đầu
                logger.info(f"💸 Casino loss: User {self.game.user_id} bet {self.game.bet_amount}, lost all")
            
        except Exception as e:
            logger.error(f"❌ Error in casino payout: {e}")

class NewCasinoCog(commands.Cog):
    """Casino mới với logic đơn giản"""
    
    def __init__(self, bot):
        self.bot = bot
        self.active_games: Dict[int, SimpleBlackjackGame] = {}
        self.cooldowns: Dict[int, datetime] = {}
        logger.info("🎰 New Casino Cog initialized")
    ...
    def _check_cooldown(self, user_id: int) -> Optional[float]:
        ...
        cooldown_seconds = config.CASINO_CONFIG.get("cooldown", 3)
    ...
    @commands.command(name='casino', aliases=['bj2'])
    @registration_required
    async def casino(self, ctx, bet_amount: int = None):
        """
        🎰 Casino Blackjack mới!
        
        Cách dùng: f!casino <số_tiền>
        Ví dụ: f!casino 1000
        """
        try:
            # Validation
            if bet_amount is None:
                embed = EmbedBuilder.create_info_embed(
                    "🎰 **CASINO BLACKJACK**",
                    f"Sử dụng: `{config.PREFIX}casino <tiền_cược>`\n"
                    f"Ví dụ: `{config.PREFIX}casino 1000`\n\n"
                    f"**Luật chơi:**\n"
            ...
            # Validate bet
            min_bet = config.CASINO_CONFIG.get("min_bet", 100)
            max_bet = config.CASINO_CONFIG.get("max_bet", 100000)
            ...
            logger.info(f"🎲 Casino bet: User {ctx.author.id} bet {bet_amount}, balance: {new_balance}")
            ...
            await ctx.send(f"❌ Lỗi casino: {str(e)}")

async def setup(bot):
    await bot.add_cog(NewCasinoCog(bot)) 